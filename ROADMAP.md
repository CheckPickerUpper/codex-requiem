...
- [ ] Create `plugins/skill-codex/scripts/codex-requiem-run.js` (or `.py`)
- [ ] Handle stderr suppression portably (no `2>/dev/null`)
- [ ] Handle prompt piping without shell-specific syntax
- [ ] Update SKILL.md to call the wrapper instead of raw `codex exec`
- [ ] Test on Windows (Pwsh), WSL, and macOS/Linux bash
